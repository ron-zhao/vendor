        ...
        <div class="viewports purchase-list">
            <div class="search-box">
                <div class="search-content">
                    <span>报价时间：</span>
                    <input type="text" v-model="query.date" @click="showPicker" placeholder="请选择报价时间" class="date-select" readonly>
                    <mt-datetime-picker
                            ref="picker"
            ...
            <div v-infinite-scroll="loadmore"
                ...
                <table class="table" border="0">
                    ...
                    <tr v-for="item in purchase" v-show="loadall">
                        <td>{{item.id}}</td>
                        <td>{{item.offer_date | date('YYYY.MM.DD')}}</td>
                        <td>{{item.product_count}}</td>
                        <td>
                            {{item.status==="finish" ?
                            "已完成" : item.status==="confirm" ?
                                    "待确认" : ""}}</td>
                        <td>
                            <a class="text-link"
                               @click="$router.push({name:'home.myPrice',query:{status:item.status, id: item.id}})">
                                查看
                            </a>
                        </td>
                    </tr>
...
<script>
    ...
    import { vendorPricesLog } from '../../api';
    ...
    export default {
        ...
        methods: {
            ...
            list () {
                this.query.offset = 0;
                this.loaded = false;
                vendorPricesLog(this.query).success(data => {
